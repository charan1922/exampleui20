...
export default function MaterialTableDemo() {
  ...
  const [state, setState] = useState({
    ...
    data: [
      { jobId: 'JOB_ID_01', jobName: 'db2_job_shashi', jobValidate: false, lastModifiedDate: '2020-04-02T09:30', scenarios: ['AnovaGPAR', 'GPAR', 'abcde', 'scenario20'], jobDesc: 'lorem ipsum covid-19' },
      { jobId: 'JOB_ID_02', jobName: 'multi_date_job_test', jobValidate: null, lastModifiedDate: '2017-06-02T11:22', scenarios: ['AnovaGPAR', 'GPAR', 'abcde'], jobDesc: 'multi_date_job_test' },
  ...
  let invalidJobs = invalidJobsData.length ? invalidJobsData.reduce((ac, cv) => `${ac} ${cv.jobName},`, '') : null;



  let validJobsData = tableData.length ? tableData.filter(item => item.jobValidate) : []
  let validJobs = validJobsData.length ? validJobsData.reduce((ac, cv) => `${ac} ${cv.jobName},`, '') : null
    ...
    <>
      ...
      <Dialog open={executeJob} onClose={() => setExecuteJob(false)} fullWidth>
        ...
        <DialogContentText id="alert-dialog-slide-description">
          <span style={{ marginBottom: '5px' }}>{invalidJobs ? `${invalidJobs} validation and exicution will be failed` : null}</span>
          <br />
          <span style={{ marginBottom: '5px' }}>{validJobs ? `${validJobs} validation and exicution will be failed` : null}</span>
        </DialogContentText>
        <DialogActions>
      ...
      <Button onClick={handleClick('top')}>Validate</Button>
      {/* <Button onClick={() => setDialogOpen(true)}>Add Scenario</Button> */}
      <Button disabled={tableData.length ? false : true} onClick={() => setExecuteJob(true)}>Execute JOB</Button>
